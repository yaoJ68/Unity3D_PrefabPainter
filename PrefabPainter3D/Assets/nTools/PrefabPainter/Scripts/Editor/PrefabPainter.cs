...
namespace nTools.PrefabPainter
{
	...
	public class PrefabPainter : EditorWindow
	{
		...
		// Control preview texture cache size
		int previewTextureCacheSize = 0;


		void SetMinimumPreviewCacheSize(int size)
		{
			if (size > previewTextureCacheSize) {
				previewTextureCacheSize = size;
				AssetPreview.SetPreviewTextureCacheSize(previewTextureCacheSize);
			}
		}
		...
		void OnEnable () 
		{
			...
			if (EditorApplication.hierarchyWindowChanged != HierarchyWindowChangedCallback)
                EditorApplication.hierarchyWindowChanged += HierarchyWindowChangedCallback;
		...
		GenericMenu GetPresetMenu()
		{
			GenericMenu menu = new GenericMenu();
			menu.AddItem(new GUIContent("Reveal in Project"), false, ContextMenuCallback, new Action(RevealPrefabInProject));
			menu.AddSeparator ("");
			menu.AddItem(new GUIContent("Copy Settings"), false, ContextMenuCallback, new Action(() => settings.ClipboardCopy()));
			if(settings.ClipboardIsCanPaste())
				menu.AddItem(new GUIContent("Paste Settings"), false, ContextMenuCallback, new Action(() => settings.ClipboardPaste()));
			else
				menu.AddDisabledItem(new GUIContent("Paste Settings"));
			menu.AddSeparator ("");
			menu.AddItem(new GUIContent("Delete"), false, ContextMenuCallback, new Action(() => settings.DeleteSelectedPresets()));
			menu.AddItem(new GUIContent("Duplicate"), false, ContextMenuCallback, new Action(() => settings.DuplicateSelectedPresets()));
			menu.AddItem(new GUIContent("Reset"), false, ContextMenuCallback, new Action(() => settings.ResetSelectedPresets()));

			return menu;
		}

		GenericMenu GetMultiPresetMenu()
		{
			GenericMenu menu = new GenericMenu();
		...
		{            
			...
			BrushPreset selectedPreset = settings.GetFirstSelectedPreset();
			int presetCount = settings.presets.Count;
			    ...
			    currentTool == PaintTool.Erase)
            {
				...
				// Draw Presets Window
                {
...

					{
						...
						int virtualRows   = Mathf.CeilToInt((float)presetCount / presetColumns);
					...
					for (int y = (int)virtualRect.yMin + windowBorder; y < (int)virtualRect.yMax; y += Styles.presetIconHeight)
                    {
						...
						for (int x = (int)virtualRect.xMin + windowBorder; (x+Styles.presetIconWidth) < (int)(virtualRect.xMax); x += Styles.presetIconWidth)
						{
							...
							if(presetIconRectScrolled.Overlaps(realRect))
							{
								...
								BrushPreset preset = settings.presets[presetIndex];
	                            ...
	                            if (preset.prefab != null) {
	                                // Prefab name
									EditorGUI.LabelField(presetIconRect, preset.GetShortName(), Styles.iconLabelText);
	                            } else {
					...
					SetMinimumPreviewCacheSize(iconDrawCount + 5);
					...
					if (e.type == EventType.MouseDown && e.button == 0)
					{
						if(presetUnderCursor != -1)
						{
							...
							if (e.control)
	#endif
								settings.SelectPresetAdd(presetUnderCursor);
							else if (e.shift) 							
								settings.SelectPresetRange(presetUnderCursor);
							else
								settings.SelectPreset(presetUnderCursor);
						}
						else
						{
					...
					if (e.type == EventType.ContextClick && presetUnderCursor != -1)
					{
						if (settings.IsPresetSelected(presetUnderCursor))
						{
							if (hasMultipleSelectedPresets)
								GetMultiPresetMenu().ShowAsContext();
							else
								GetPresetMenu().ShowAsContext();
						}
						else
						{
							settings.SelectPreset(presetUnderCursor);
							...
							if (e.type == EventType.DragPerform) {
								...
								foreach (UnityEngine.Object draggedObject in DragAndDrop.objectReferences)
								{
									    ...
									    AssetDatabase.Contains(draggedObject))
									{
										...
										settings.presets[presetUnderCursor].AssignPrefab(draggedObject as GameObject);                                                        
									}
								}
							}
							e.Use();
						}
						else if(virtualRect.Contains (e.mousePosition))
							// Add Prefab
                        {
                            ...
                            if (e.type == EventType.DragPerform) {
                                ...
                                foreach (UnityEngine.Object draggedObject in DragAndDrop.objectReferences)
                                {
                                        ...
                                        AssetDatabase.Contains(draggedObject))
                                    {
                                        ...
                                        settings.presets.Add(new BrushPreset(draggedObject as GameObject));                                        
                                    }
                                }
                            }
                            e.Use();
				...
				else if (hasSelectedPresets && selectedPreset != null)
                {
					...
					positionSettingsFoldout = EditorGUILayout.Foldout(positionSettingsFoldout, "Preset Settings", Styles.boldFoldout);
					if (positionSettingsFoldout)
					{
	                    ++EditorGUI.indentLevel;
						selectedPreset.SetName(EditorGUILayout.TextField ("Preset name", selectedPreset.GetName()));
						if (currentTool != PaintTool.Erase) {
                    ...
                    {                      
						...
						positionSettingsFoldout = EditorGUILayout.Foldout(positionSettingsFoldout, "Position", Styles.boldFoldout);
                        if (positionSettingsFoldout)
                        {
                            ++EditorGUI.indentLevel;
						...
						orientationSettingsFoldout = EditorGUILayout.Foldout(orientationSettingsFoldout, "Orientation", Styles.boldFoldout);
						...
						scaleSettingsFoldout = EditorGUILayout.Foldout(scaleSettingsFoldout, "Scale", Styles.boldFoldout);
				...
				commonSettingsFoldout = EditorGUILayout.Foldout(commonSettingsFoldout, "Common Settings", Styles.boldFoldout);
            ...
            if (currentTool == PaintTool.Settings)
            {
                ...
                ++EditorGUI.indentLevel;
				settings.brushSizeMax = Mathf.Max(EditorGUILayout.FloatField("Max Brush Size", settings.brushSizeMax), 0.5f);
				settings.brushSpacingMax = Mathf.Max(EditorGUILayout.FloatField("Max Brush Spacing", settings.brushSpacingMax), 0.5f);
				settings.precisePlaceSnapAngle = Mathf.Max(EditorGUILayout.FloatField("Precise Place Snap Angle", settings.precisePlaceSnapAngle), 1.0f);
				settings.surfaceCoords = (SurfaceCoords) EditorGUILayout.EnumPopup("Along Surface Coords", settings.surfaceCoords);
                --EditorGUI.indentLevel;
        ...
        GameObject Raycast(Vector2 mousePosition, out RaycastHitEx raycastHit, int layersMask)
        {
		...
		#else
			GameObject gameObject = HandleUtility.PickGameObject(mousePosition, false, placedObjects);
		#endif
			if (gameObject == null)
				return null;
            ...
            if (meshFilter != null && meshFilter.sharedMesh != null)
            {
				...
				if(Utility.IntersectRayMeshEx (ray, meshFilter.sharedMesh, meshFilter.transform.localToWorldMatrix, out raycastHit))
                {
			...
			else if ((rectTransform = gameObject.GetComponent<RectTransform>()) != null)
			{
				...
				Plane rectPlane = new Plane(fourCorners[0], fourCorners[1], fourCorners[2]);

				float distance = 0;

				if(rectPlane.Raycast(ray, out distance))
				{
					raycastHit.ray = ray;
					raycastHit.isHit = true;
					raycastHit.distance = distance;
					raycastHit.point = ray.GetPoint(distance);
					raycastHit.normal = rectPlane.normal;
		...
		void ParentObject(GameObject gameObject, BrushPreset preset)
        {
            ...
            if(settings.groupPrefabs)
            {
				...
				string groupName = preset.GetName() + "_group";
			...
			if (gameObject != null && parentObject != null && parentObject.transform != null)
				gameObject.transform.SetParent(parentObject.transform, true);
        }
        ...
        void DrawEraseHandles(RaycastHitEx hit, BrushPreset preset)
        {
			...
			DrawXYZCross(hit, right, forward);
        }
        ...
        void DrawPrecisePlaceHandles(RaycastHitEx hit, float radius, float angle, Vector3 point)
        {
			...
			Handles.color = Color.red;
			Handles.Label(point, "    A: " + (angle).ToString("F2"), Styles.precisePlaceTextStyle);
			Handles.Label(point, "\n    R: " + (radius).ToString("F2"), Styles.precisePlaceTextStyle);
		...
		void DrawXYZCross(RaycastHitEx hit, Vector3 right, Vector3 forward)
		{
			float handleSize = HandleUtility.GetHandleSize (hit.point) * 0.5f;

			Handles.color = Color.green;
			Handles.DrawLine(hit.point + hit.normal * handleSize, hit.point + hit.normal * -handleSize * 0.2f);
			Handles.color = Color.red;
			Handles.DrawLine(hit.point + right * handleSize, hit.point + right * -handleSize * 0.2f);
			Handles.color = Color.blue;
			Handles.DrawLine(hit.point + forward * handleSize, hit.point + forward * -handleSize * 0.2f);
		}
                ...
                if (onPrecisePlace)
                {
                    ...
                    if (onMouseDown)
                    {
                        ...
                        if (hitInfo.isHit)
                        {
							...
							preciseGameObject.transform.localScale = Vector3.zero;
                        }
                        preciseHitInfo = hitInfo;
                    }

						if (preciseHitInfo.isHit && preciseGameObject != null && e.type == EventType.Repaint)
                    {
                        float rayDistance;
                        ...
                        {								
							...
							if ((eventModifiers & EventModifiers.Control) != 0)
								angle = (float)((int)(Mathf.Round(angle / settings.precisePlaceSnapAngle) * settings.precisePlaceSnapAngle));
